<%@ WebHandler Language="C#" Class="downloadReportBureau" %>
...
public class downloadReportBureau : ReferenceClass, IHttpHandler
{
    ...
    public string Combine(HttpContext context)
    {
        ...
        string gover_type = HttpContext.Current.Request["gover_type"];
        ...
        if (mlist.Count > 0)
        {
            ...
            //是否为导出周报 机车版本
            if (Api.Util.Common.FunEnable("Fun_isCRH") == false)
            {
                if (orgcode == "TOPBOSS")
                    orgcode = HttpContext.Current.Request["orgCode"];
                if (orgname == "铁路总公司")
                    orgname = HttpContext.Current.Request["orgName"];
                //是否包含机务段信息,包含则改变文档名称
                if (!string.IsNullOrEmpty(HttpContext.Current.Request["jwd"]) && HttpContext.Current.Request["jwd"] != "0")
                {
                    orgcode = HttpContext.Current.Request["jwd"];
                    orgname = HttpContext.Current.Request["duanText"];
                }
            }
            ...
            //生成周报的统计版块
            export(time, orgname, orgcode, ref text_type_in, ref text_type_out, ref text_line_in, ref text_line_out, ref text_locodate, ref dective_count, ref out_count);
            ...
            {
                if (out_count != 0)//如果非本局统计的总数为0，则不显示二级标题
                {
                ...
                if (dective_count > 7)//如果车辆运行情况统计超过7辆车，则分页显示
                {
                    word.InsertBreak(1);
                }
                ...
                if (out_count != 0)//如果非本局统计的总数为0，则非本局部分不显示
                {
            ...
            try
            {
                for (int i = 0; i < mlist.Count; i++)
                {
                    ...
                    if (i == 0)
                    {
                        ...
                        if (mlist[i].SVALUE15 == "重复报警")
                        {
                            string repeatpath = repeat(mlist[i].ID, alarmCond.startTime, alarmCond.endTime);
                            if (i != mlist.Count - 1)
                            {
            ...
            if (DateTime.Parse(alarmCond.startTime.ToString("HH:mm:ss")) >= DateTime.Parse("17:30:00"))
                alarmCond.startTime = alarmCond.startTime.AddDays(1);

            //是否为导出周报 机车版本
            if (Api.Util.Common.FunEnable("Fun_isCRH") == false)
            {
                //机车周报WORD文件名称
                name = "机车车载接触网运行状态检测装置（3C）" + orgname + "疑似缺陷周报（" + alarmCond.startTime.ToString("yyyy.MM.dd") + "-" + alarmCond.endTime.ToString("yyyy.MM.dd") + "）.doc";
            }
            else
            {
                //动车周报WORD文件名称
                name = "动车组车载接触网运行状态检测装置（3C）" + orgname + "疑似缺陷周报（" + alarmCond.startTime.ToString("yyyy.MM.dd") + "-" + alarmCond.endTime.ToString("yyyy.MM.dd") + "）.doc";
            }

            //周报WORD文件全路径名称
            string filename = System.Web.HttpContext.Current.Server.MapPath(@"~/TempReport/") + name;
            //word.Delete(filename + ".doc");//保证周报是重新生成
    ...
    /// <summary>
    /// 生成周报的概述部分（动车生成4个图表，机车生成1个图表）
    /// </summary>
    /// <param name="time">日期</param>
    /// <param name="orgname">组织名称</param>
    /// <param name="orgcode">组织编码</param>
    /// <param name="text_type_in">本局典型的文本</param>
    /// <param name="text_type_out">非本局典型的文本</param>
    /// <param name="text_line_in">本局线路的文本</param>
    /// <param name="text_line_out">非本局线路的文本</param>
    /// <param name="text_locodate">车辆运行情况的文本</param>
    /// <param name="dective_count">车辆运行情况的车辆数</param>
    /// <param name="out_count">非本局的总报警数</param>
    private void export(string time, string orgname, string orgcode, ref string text_type_in, ref string text_type_out, ref string text_line_in, ref string text_line_out, ref StringBuilder text_locodate, ref int dective_count, ref int out_count)
    {
        string gover_type = HttpContext.Current.Request["gover_type"];
        ...
        if (gover_type == "cld")
        {
            string posi = aa.GetPosi_in_Jc(Getstrwhere(), ju);
            ds_type_in = aa.GetType_in_Jc(Getstrwhere(), ju, ref totalcount_type_in, ref loco_in);
            ds_chart_type_in = aa.GetChar_type_in_Jc(Getstrwhere(), ju, ref totalcount_chart_type_in);
            text_type_in = string.Format(@"{0}各机车在{1}主要运行区间：{2}；主要运行线路为：{3}。共检测出缺陷{4}条，其中典型缺陷共计{5}条，统计结果如下图所示：", orgname, date, posi, loco_in, totalcount_type_in, totalcount_chart_type_in);
        ...
        else
        {
            ...
            text_line_in = string.Format(@"{0}主要运行线路为 {1}；在管辖范围内检测疑似缺陷情况：{2}。线路缺陷分布情况如下图表所示：", date, line_in, line_in_count);

            text_line_out = string.Format(@"{0}，{1}；共发现{2}条缺陷，线路缺陷分布情况如下图表所示：", date, line_out, line_out_count);

            out_count = totalcount_type_out;//获取非本局统计的报警总数
    ...
    private string repeat(string id, DateTime start, DateTime end)
    {
        ...
        if (!File.Exists(filename))
        {
            ...
            {
                log4net.ILog log = log4net.LogManager.GetLogger("生成重复报表");
                log.Error("执行出错", ex);
            }
        }
        return filename;
    ...
    private string Gettextdate(string startdate, string enddate)
    {
        ...
        if (DateTime.Parse(start.ToString("HH:mm:ss")) >= DateTime.Parse("17:30:00"))
        {
            s = start.Year.ToString() + "年" + start.Month.ToString() + "月" + (start.Day + 1).ToString() + "日";
        }
    ...
    private DataTable getDataTable(IList<C3_Alarm> c3List, DateTime start, DateTime end)
    {
        ...
        DataColumn dc19 = new DataColumn("ORG_SHOW", Type.GetType("System.String"));
        DataColumn dc20 = new DataColumn("NULL_PAGE", Type.GetType("System.String"));
        ...
        if (c3List.Count > 0)
        {
            C3_Alarm c3Alarm = c3List[c3List.Count - 1];
            ...
            dr["NULL_PAGE"] = "http://" + HttpContext.Current.Request.Url.Host + ":" + HttpContext.Current.Request.Url.Port + "/Report/NULL.jpg";//为了达到重复报表中 第一个表格 与  第二个表格之间对齐，而在第一个表格上了加入一行插入图像NULL.jpg（图像为空白图像，行距很小，基本不影响生成WORD的展示）
            ...
            dr["LC_VAULE1"] = "false";
            dr["LC_VAULE2"] = "false";
            dr["ORG_SHOW"] = "true";//判断是否为机车数据
            if (c3List.Count > 0 && c3List[0].DETECT_DEVICE_CODE.IndexOf("CRH") < 0)
            {
                dr["ORG_SHOW"] = "false";
            }
    ...
    private DataTable getDetailDataTable(IList<C3_Alarm> c3List)
    {
        ...
        foreach (C3_Alarm c3Alarm in c3List)
        {
            ...
            //string irv = "http://192.168.1.130:10020/" + dir + c3Alarm.SNAPPED_IMA.Replace(".IMA", "_IRV.jpg");
            //string jpg = "http://192.168.1.130:10020/" + dir + c3Alarm.SNAPPED_JPG;
            //string ov = "http://192.168.1.130:10020/" + dir + Convert_Url(c3Alarm.SVALUE9);

            string irv = "http://" + HttpContext.Current.Request.Url.Host + ":" + HttpContext.Current.Request.Url.Port + dir + c3Alarm.SNAPPED_IMA.Replace(".IMA", "_IRV.jpg");
            string jpg = "http://" + HttpContext.Current.Request.Url.Host + ":" + HttpContext.Current.Request.Url.Port + dir + c3Alarm.SNAPPED_JPG;
            string ov = "http://" + HttpContext.Current.Request.Url.Host + ":" + HttpContext.Current.Request.Url.Port + dir + Convert_Url(c3Alarm.SVALUE9);
    ...
    /// <summary>
    /// 将为数字0 转换成 英文字母o  (RDLC控件原因，无法读取单个的0)
    /// </summary>
    /// <param name="re"></param>
    /// <returns></returns>

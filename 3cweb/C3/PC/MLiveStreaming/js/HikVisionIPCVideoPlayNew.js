...
$(function () {
    ...
    var winh = window.screen.height * 0.44; //高
    var winw = window.screen.width * 0.49; //宽
    if (window.screen.height != 1200) {
        winh = window.screen.height * 0.42; //高
   ...
   // document.getElementById("divPlugin").style.display = "none";
    ...
    if (GetQueryString("Loco") != undefined && GetQueryString("Loco") != "") {
        showPlugin();
        clickRealPlay2(GetQueryString("Loco"));
    }
});
...
function doQuery() {
    ...
    var url = '/Common/MFoundation/RemoteHandlers/LocomotiveControl.ashx?type=all' + '&LOCOMOTIVE_CODE=' + escape(loccode);
...
function showOPInfo(szInfo) {
    szInfo = "<p style='color: white'>" + szInfo + "</p>";
...
function clickLocoCRH2229A() { $("#locomotive_no").html('CRH2229A'); }
function clickLocoCRH2237() { $("#locomotive_no").html('CRH2237'); }
function clickLocoCRH380A6145() { $("#locomotive_no").html('CRH380A6145'); }
//点击直播---
...
//点击直播---
function clickRealPlay2(loco) {

    showOPInfo("<img src='/Common/img/loading1.gif' width='30' height='30' align='absmiddle' />获取播放参数")
    playLoco = loco; // $("#locomotive_no").text();

    $("#locomotive_no").html(playLoco);
...
//获取指定设备指定端的连接信息
function startRealPlay(locomotiveCode) {

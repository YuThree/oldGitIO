...
function loadFlexiGrid(treeName, LocCode, treeType) {
    option = {
        ...
        colModel: [
                            ...
                            { display: '安装位置', name: 'SBWZ', width: 80, sortable: false, hide: true, align: 'center' },
                            ...
                            { display: '供电段电话区号', name: 'DISTRICT_CODE', width: 100, sortable: false, align: 'center' },
...
function doQuery() {
    ...
    var substation = $("#sub").val();
    var name = $("#Name").val();
    option.url = 'RemoteHandlers/C6_DeviceListControl.ashx?type=all&Name=' + escape(name) + "&ju=" + escape(ju) + "&duan=" + escape(duan) + "&substation=" + escape(substation);
    option.newp = 1;
    $("#flexTable").flexOptions(option).flexReload();

};
//树查询
function doTreeQuery(treeType, id, pId) {
    option.url = 'RemoteHandlers/C6_DeviceListControl.ashx?type=all&treeType=' + escape(treeType) + "&id=" + escape(id) + "&pId=" + pId + '&temp=' + Math.random();
    option.newp = 1;
    $("#flexTable").flexigrid(option);
    $("#flexTable").flexOptions(option).flexReload();

...
(function (window) {
    ...
    var WebVideoCtrl = (function () {
        ...
        var m_szVersion = "<?xml version='1.0' encoding='utf-8'?><FileVersion>" +
            "<Platform name='win32'>" +
            "<npWebVideoPlugin.dll>3,0,5,52</npWebVideoPlugin.dll>" +
            "<WebVideoActiveX.ocx>3,0,5,52</WebVideoActiveX.ocx>" +
            "<PlayCtrl.dll>6,5,1,6</PlayCtrl.dll>" +
            "<StreamTransClient.dll>1,1,2,8</StreamTransClient.dll>" +
            "<SystemTransform.dll>2,3,0,5</SystemTransform.dll>" +
            "<NetStream.dll>1,0,4,52</NetStream.dll>" +
        ...
        window.ZoomInfoCallback = function (szZoomInfo) {
            ...
            if(iIndex != -1) {
                ...
                if(iIndex != -1) {
                    oDeviceInfo = m_deviceSet[iIndex];
            ...
            deviceInfo.oProtocolInc.getPortInfo(deviceInfo, {
                ...
                success: function (xmlDoc) {
                    ...
                    for (var i = 0, iLen = nodeList.length; i < iLen; i++) {
                        if(NS.$XML(nodeList).eq(i).find("protocol").eq(0).text() == "RTSP") {
                            iRtspPort = parseInt(NS.$XML(nodeList).eq(i).find("portNo").eq(0).text(), 10);
                        }
                        if(NS.$XML(nodeList).eq(i).find("protocol").eq(0).text() == "DEV_MANAGE") {
        ...
        var _getExternalPort = function (deviceInfo) {
            ...
            deviceInfo.oProtocolInc.getUPnPPortStatus(deviceInfo, {
                ...
                success: function (xmlDoc) {
                    ...
                    for (var i = 0, iLen = nodeList.length; i < iLen; i++) {
                        if(NS.$XML(nodeList).eq(i).find("internalPort").eq(0).text() == "rtsp") {
                            iRtspPort = parseInt(NS.$XML(nodeList).eq(i).find("externalPort").eq(0).text(), 10);
                        }
                        if(NS.$XML(nodeList).eq(i).find("internalPort").eq(0).text() == "admin") {
        ...
        /*************************************************
         ...
         return:            无
         *************************************************/
        this.I_Login = function (szIP, iPrototocol, iPort, szUserName, szPassword, options) {
            ...
            var newOptions = {
                protocol: iPrototocol,
                success: null,
                error: null
            };

            var szAuth = m_utilsInc.Base64.encode(szUserName + ":" + szPassword);
            ...
            m_utilsInc.extend(newOptions, {
                success: function (xmlDoc) {
                    ...
                    deviceInfo.szHttpProtocol = iPrototocol == 2 ? "https://" : "http://";
                    deviceInfo.iHttpPort = iPort;
                    deviceInfo.szAuth = szAuth;
                    deviceInfo.iDeviceProtocol = cgiType;
                    deviceInfo.oProtocolInc = cgiInstance;  //此处把协议索引赋值给设备对象，设备以后的交互都将使用这个对象
                    m_deviceSet.push(deviceInfo);
                    _PrintString("使用%s协议登录成功", cgiType);
                ...
                error: function (httpStatus, xmlDoc) {
                    ...
                    m_utilsInc.extend(newOptions, {
                        success: function (xmlDoc) {
                            ...
                            deviceInfo.iHttpPort = iPort;
                            deviceInfo.szAuth = szAuth;
                            deviceInfo.iDeviceProtocol = PROTOCOL_DEVICE_PSIA;
                            deviceInfo.oProtocolInc = m_PSIAProtocol;
                            m_deviceSet.push(deviceInfo);
                            _PrintString("使用%s协议登录成功", "PSIA");
                    ...
                    m_PSIAProtocol.login(szIP, iPort, szAuth, newOptions);
                }
            });

            cgiInstance.login(szIP, iPort, szAuth, newOptions);
        };
        ...
        this.I_StartRealPlay = function (szIP, options) {
            ...
            if(iIndex != -1) {
                ...
                //优先使用用户传入的端口号
                if(options.iPort != undefined) {
                    deviceInfo.iRtspPort = options.iPort;
                }
        ...
        this.I_Stop = function (iWndIndex) {
            iWndIndex = !!iWndIndex ? iWndIndex : m_iSelWnd;
        ...
        /*************************************************
         ...
         Description:    云台控制
         Input:            iPTZIndex：1：上 2：下 3：左 4：右 5：左上 6：左下 7：右上 8：右下 9：自动 10：停止
         options：可选参数
         Output:        无
         return:        无
         *************************************************/
        this.I_PTZControl = function (iPTZIndex, options) {
            var newOptions = {
                iWndIndex: m_iSelWnd,
                iPTZIndex: iPTZIndex,
                iPTZSpeed: 4
            };
            if(options.async) {//云台强制同步
                options.async = false;
            }
            m_utilsInc.extend(newOptions, options);
            ...
            if(iIndex != -1) {
                ...
                if(iIndex != -1) {
                    ...
                    if(9 == iPTZIndex) {//自动
                        deviceInfo.oProtocolInc.ptzAutoControl(deviceInfo, wndInfo, newOptions);
                    } else {
                        deviceInfo.oProtocolInc.ptzControl(deviceInfo, wndInfo, newOptions);
                    }
                }
            }
        };
        ...
        /*************************************************
         ...
         return:        无
         *************************************************/
        this.I_StartDownloadRecord = function (szIP, szPlaybackURI, szFileName, options) {
            var iIndex = this.findDeviceIndexByIP(szIP),
                iRet = -1;
            if(iIndex != -1) {
                var deviceInfo = m_deviceSet[iIndex];
        ...
        this.I_RestoreDefault = function (szIP, szMode, options) {
            ...
            if(iIndex != -1) {
                ...
                deviceInfo.oProtocolInc.restore(deviceInfo, szMode, options);
            }
        };
        ...
        this.I_Restart = function (szIP, options) {
            ...
            if(iIndex != -1) {
                ...
                deviceInfo.oProtocolInc.restart(deviceInfo, options);
            }
        };
        ...
        this.I_Reconnect = function (szIP, options) {
            ...
            if(iIndex != -1) {
                ...
                deviceInfo.oProtocolInc.login(deviceInfo.szIP, deviceInfo.iHttpPort, deviceInfo.szAuth, options);
            }
        };
        ...
        this.I_RemoteConfig = function (szIP, options) {
            ...
            if(iIndex != -1) {
                ...
                //表示没有获取过设备端口号，需要获取一次
                if(-1 == deviceInfo.iDevicePort && ("undefined" == typeof options.iDevicePort || "" == options.iDevicePort)) {
                ...
                iDevicePort = options.iDevicePort || deviceInfo.iDevicePort;
                ...
                szUserName = m_utilsInc.Base64.decode(deviceInfo.szAuth).split(":")[0];
                szPassword = m_utilsInc.Base64.decode(deviceInfo.szAuth).split(":")[1];
        ...
        deviceInfoClass.prototype.getMyDeviceInfo = function () {
        ...
        HttpPluginClient.prototype.httpDataAnalyse = function (oHttpRequest, szHttpContent) {
            ...
            //如果没有任何数据，表示请求失败
            if("" == szHttpContent || undefined == szHttpContent) {
                //考虑调用插件的接口来获取错误
        ...
        ISAPIProtocol.prototype.CGI = {
            ...
            getStreamChannels: "%s%s:%s/PSIA/Streaming/channels",   //没找到对应协议，没有使用过
        ...
        ISAPIProtocol.prototype.getAudioInfo = function (deviceInfo, options) {
            //组成url
            var szUrl = _FormatString(this.CGI.getAudioInfo, deviceInfo.szHttpProtocol, deviceInfo.szIP, deviceInfo.iHttpPort);
        ...
        ISAPIProtocol.prototype.getDeviceInfo = function (deviceInfo, options) {
            //组成url
            var url = _FormatString(this.CGI.getDeviceInfo, deviceInfo.szHttpProtocol, deviceInfo.szIP, deviceInfo.iHttpPort);
        ...
        ISAPIProtocol.prototype.getAnalogChannelInfo = function (deviceInfo, options) {
            //组成url
            var url = _FormatString(this.CGI.getAnalogChannelInfo, deviceInfo.szHttpProtocol, deviceInfo.szIP, deviceInfo.iHttpPort);
        ...
        ISAPIProtocol.prototype.getDigitalChannelInfo = function (deviceInfo, options) {
            //组成url
            var url = _FormatString(this.CGI.getDigitalChannelInfo, deviceInfo.szHttpProtocol, deviceInfo.szIP, deviceInfo.iHttpPort);
        ...
        ISAPIProtocol.prototype.getZeroChannelInfo = function (deviceInfo, options) {
            //组成url
            var url = _FormatString(this.CGI.getZeroChannelInfo, deviceInfo.szHttpProtocol, deviceInfo.szIP, deviceInfo.iHttpPort);
        ...
        ISAPIProtocol.prototype.getPPPoEStatus = function (deviceInfo, options) {
            //组成url
            var url = _FormatString(this.CGI.getPPPoEStatus, deviceInfo.szHttpProtocol, deviceInfo.szIP, deviceInfo.iHttpPort);
        ...
        ISAPIProtocol.prototype.getUPnPPortStatus = function (deviceInfo, options) {
            //组成url
            var url = _FormatString(this.CGI.getUPnPPortStatus, deviceInfo.szHttpProtocol, deviceInfo.szIP, deviceInfo.iHttpPort);
        ...
        ISAPIProtocol.prototype.getNetworkBond = function (deviceInfo, options) {
            //组成url
            var url = _FormatString(this.CGI.getNetworkBond, deviceInfo.szHttpProtocol, deviceInfo.szIP, deviceInfo.iHttpPort);
        ...
        ISAPIProtocol.prototype.getPortInfo = function (deviceInfo, options) {
            //组成url
            var url = _FormatString(this.CGI.getPortInfo, deviceInfo.szHttpProtocol, deviceInfo.szIP, deviceInfo.iHttpPort);
        ...
        ISAPIProtocol.prototype.startRealPlay = function (oDeviceInfo, options) {
            ...
            if(options.bZeroChannel) {
                szUrl = _FormatString(this.CGI.startRealPlay.zeroChannels, "rtsp://", oDeviceInfo.szIP, oDeviceInfo.iRtspPort, iChannelID);
            } else {
                szUrl = _FormatString(this.CGI.startRealPlay.channels, "rtsp://", oDeviceInfo.szIP, oDeviceInfo.iRtspPort, iChannelID);
            }
        ...
        ISAPIProtocol.prototype.startVoiceTalk = function (oDeviceInfo, iAudioChannel) {
            //组成url
            var szOpenUrl = _FormatString(this.CGI.startVoiceTalk.open, oDeviceInfo.szHttpProtocol, oDeviceInfo.szIP, oDeviceInfo.iHttpPort, iAudioChannel),
                szCloseUrl = _FormatString(this.CGI.startVoiceTalk.close, oDeviceInfo.szHttpProtocol, oDeviceInfo.szIP, oDeviceInfo.iHttpPort, iAudioChannel),
                szAudioDataUrl = _FormatString(this.CGI.startVoiceTalk.audioData, oDeviceInfo.szHttpProtocol, oDeviceInfo.szIP, oDeviceInfo.iHttpPort, iAudioChannel);
        ...
        ISAPIProtocol.prototype.ptzAutoControl = function (oDeviceInfo, oWndInfo, options) {
            ...
            if(oDeviceInfo.szDeviceType != "IPDome") {
                if(iChannelID <= oDeviceInfo.iAnalogChannelNum) {
                    szUrl = _FormatString(this.CGI.ptzAutoControl.analog, oDeviceInfo.szHttpProtocol, oDeviceInfo.szIP, oDeviceInfo.iHttpPort, oWndInfo.iChannelID);
                } else {
                    szUrl = _FormatString(this.CGI.ptzAutoControl.digital, oDeviceInfo.szHttpProtocol, oDeviceInfo.szIP, oDeviceInfo.iHttpPort, oWndInfo.iChannelID);
                }
                ...
                var iPresetID = 99;
                if(0 == options.iPTZSpeed) {
                    iPresetID = 96;
                }
                szUrl = _FormatString(this.CGI.ptzAutoControl.ipdome, oDeviceInfo.szHttpProtocol, oDeviceInfo.szIP, oDeviceInfo.iHttpPort, oWndInfo.iChannelID, iPresetID);
            }
        ...
        ISAPIProtocol.prototype.ptzControl = function (oDeviceInfo, oWndInfo, options) {
            var iChannelID = oWndInfo.iChannelID,
                szUrl = "";

            if(oWndInfo.bPTZAuto) {
                this.ptzAutoControl(oDeviceInfo, oWndInfo, {iPTZSpeed: 0});
            }

            if(iChannelID <= oDeviceInfo.iAnalogChannelNum) {
                szUrl = _FormatString(this.CGI.ptzControl.analog, oDeviceInfo.szHttpProtocol, oDeviceInfo.szIP, oDeviceInfo.iHttpPort, oWndInfo.iChannelID);
            } else {
                szUrl = _FormatString(this.CGI.ptzControl.digital, oDeviceInfo.szHttpProtocol, oDeviceInfo.szIP, oDeviceInfo.iHttpPort, oWndInfo.iChannelID);
            }
            ...
            var szData = "<?xml version='1.0' encoding='UTF-8'?>" +
                "<PTZData>" +
        ...
        ISAPIProtocol.prototype.setPreset = function (oDeviceInfo, oWndInfo, options) {
            ...
            if(iChannelID <= oDeviceInfo.iAnalogChannelNum) {
                szUrl = _FormatString(this.CGI.setPreset.analog, oDeviceInfo.szHttpProtocol, oDeviceInfo.szIP, oDeviceInfo.iHttpPort, oWndInfo.iChannelID, options.iPresetID);
            } else {
                szUrl = _FormatString(this.CGI.setPreset.digital, oDeviceInfo.szHttpProtocol, oDeviceInfo.szIP, oDeviceInfo.iHttpPort, oWndInfo.iChannelID, options.iPresetID);
            }
        ...
        ISAPIProtocol.prototype.goPreset = function (oDeviceInfo, oWndInfo, options) {
            ...
            if(iChannelID <= oDeviceInfo.iAnalogChannelNum) {
                szUrl = _FormatString(this.CGI.goPreset.analog, oDeviceInfo.szHttpProtocol, oDeviceInfo.szIP, oDeviceInfo.iHttpPort, oWndInfo.iChannelID, options.iPresetID);
            } else {
                szUrl = _FormatString(this.CGI.goPreset.digital, oDeviceInfo.szHttpProtocol, oDeviceInfo.szIP, oDeviceInfo.iHttpPort, oWndInfo.iChannelID, options.iPresetID);
            }
        ...
        ISAPIProtocol.prototype.recordSearch = function (oDeviceInfo, options) {
            ...
            szUrl = _FormatString(this.CGI.recordSearch, oDeviceInfo.szHttpProtocol, oDeviceInfo.szIP, oDeviceInfo.iHttpPort);
            ...
            szData = "<?xml version='1.0' encoding='UTF-8'?>" +
                ...
                "<metadataList>" +
                "<metadataDescriptor>//metadata.ISAPI.org</metadataDescriptor>" +
                "</metadataList>" +
                "</CMSearchDescription>";
        ...
        ISAPIProtocol.prototype.startPlayback = function (oDeviceInfo, options) {
            var iChannelID = options.iChannelID * 100 + 1,
                iWndIndex = options.iWndIndex,
                szStartTime = options.szStartTime,
                szEndTime = options.szEndTime;

            //组成url
            var szUrl = _FormatString(this.CGI.startPlayback, "rtsp://", oDeviceInfo.szIP, oDeviceInfo.iRtspPort, iChannelID, szStartTime, szEndTime);
        ...
        ISAPIProtocol.prototype.startDownloadRecord = function (oDeviceInfo, options) {
            //组成url
            var szUrl = _FormatString(this.CGI.startDownloadRecord, oDeviceInfo.szHttpProtocol, oDeviceInfo.szIP, oDeviceInfo.iHttpPort);

            var szDownXml = "<?xml version='1.0' encoding='UTF-8'?>" +
                "<downloadRequest>" +
                "<playbackURI> " + options.szPlaybackURI + "</playbackURI>" +
                "</downloadRequest>";
        ...
        ISAPIProtocol.prototype.exportDeviceConfig = function (oDeviceInfo) {
            //组成url
            var szUrl = _FormatString(this.CGI.deviceConfig, oDeviceInfo.szHttpProtocol, oDeviceInfo.szIP, oDeviceInfo.iHttpPort);
        ...
        ISAPIProtocol.prototype.restart = function (oDeviceInfo, options) {
            //组成url
            var szUrl = _FormatString(this.CGI.restart, oDeviceInfo.szHttpProtocol, oDeviceInfo.szIP, oDeviceInfo.iHttpPort);
        ...
        ISAPIProtocol.prototype.restore = function (oDeviceInfo, szMode, options) {
            //组成url
            var szUrl = _FormatString(this.CGI.restore, oDeviceInfo.szHttpProtocol, oDeviceInfo.szIP, oDeviceInfo.iHttpPort, szMode);
        ...
        ISAPIProtocol.prototype.startUpgrade = function (oDeviceInfo, options) {
            //组成url
            var szUpgradeURL = _FormatString(this.CGI.startUpgrade.upgrade, oDeviceInfo.szHttpProtocol, oDeviceInfo.szIP, oDeviceInfo.iHttpPort),
                szStatusURL = _FormatString(this.CGI.startUpgrade.status, oDeviceInfo.szHttpProtocol, oDeviceInfo.szIP, oDeviceInfo.iHttpPort);
        ...
        ISAPIProtocol.prototype.set3DZoom = function (oDeviceInfo, oWndInfo, szZoomInfo, options) {
            //组成url
            var szUrl = _FormatString(this.CGI.set3DZoom, oDeviceInfo.szHttpProtocol, oDeviceInfo.szIP, oDeviceInfo.iHttpPort, oWndInfo.iChannelID);
        ...
        PSIAProtocol.prototype.getNetworkInterface = function (deviceInfo, options) {
            //组成url
            var url = _FormatString(this.CGI.getNetworkInterface, deviceInfo.szHttpProtocol, deviceInfo.szIP, deviceInfo.iHttpPort);
                    ...
                    m_PSIAProtocol.getStreamChannels(deviceInfo, {
                        ...
                        success: function (xmlDoc) {
                            if(NS.$XML(xmlDoc).find("rtspPortNo", true).length > 0) {
                                iRtpsPort = parseInt(NS.$XML(xmlDoc).find("rtspPortNo").eq(0).text(), 10);
                                arrXml.push("<AdminAccessProtocol>");
                                arrXml.push("<id>" + 4 + "</id>");
        ...
        PSIAProtocol.prototype.getStreamChannels = function (deviceInfo, options) {
            //组成url
            var url = _FormatString(this.CGI.getStreamChannels, deviceInfo.szHttpProtocol, deviceInfo.szIP, deviceInfo.iHttpPort);
        ...
        PSIAProtocol.prototype.getStreamDynChannels = function (deviceInfo, options) {
            //组成url
            var url = _FormatString(this.CGI.getStreamDynChannels, deviceInfo.szHttpProtocol, deviceInfo.szIP, deviceInfo.iHttpPort);
        ...
        PSIAProtocol.prototype.ptzAutoControl = function (oDeviceInfo, oWndInfo, options) {
            var iChannelID = oWndInfo.iChannelID,
                szUrl = "",
                szData = "";

            szUrl = _FormatString(this.CGI.ptzAutoControl, oDeviceInfo.szHttpProtocol, oDeviceInfo.szIP, oDeviceInfo.iHttpPort, oWndInfo.iChannelID);
        ...
        PSIAProtocol.prototype.ptzControl = function (oDeviceInfo, oWndInfo, options) {
            var iChannelID = oWndInfo.iChannelID,
                szUrl = "";
            ...
            szUrl = _FormatString(this.CGI.ptzControl, oDeviceInfo.szHttpProtocol, oDeviceInfo.szIP, oDeviceInfo.iHttpPort, oWndInfo.iChannelID);
        ...
        PSIAProtocol.prototype.setPreset = function (oDeviceInfo, oWndInfo, options) {
            ...
            szUrl = _FormatString(this.CGI.setPreset, oDeviceInfo.szHttpProtocol, oDeviceInfo.szIP, oDeviceInfo.iHttpPort, oWndInfo.iChannelID, options.iPresetID);
        ...
        PSIAProtocol.prototype.goPreset = function (oDeviceInfo, oWndInfo, options) {
            ...
            szUrl = _FormatString(this.CGI.goPreset, oDeviceInfo.szHttpProtocol, oDeviceInfo.szIP, oDeviceInfo.iHttpPort, oWndInfo.iChannelID, options.iPresetID);
        ...
        PSIAProtocol.prototype.recordSearch = function (oDeviceInfo, options) {
            ...
            szData = "<?xml version='1.0' encoding='UTF-8'?>" +
                ...
                "<searchResultPostion>" + options.iSearchPos + "</searchResultPostion>" +
                "<metadataList>" +
                "<metadataDescriptor>//metadata.psia.org</metadataDescriptor>" +
                "</metadataList>" +
                "</CMSearchDescription>";
        ...
        if(typeof window.attachEvent != "object" && m_utilsInc.browser().msie) {
            ...
            ObjectString += "<script for=" + m_szPluginID + " event='ZoomInfoCallback(ZoomInfoCallback)'>ZoomInfoCallback(ZoomInfoCallback);</script>";
